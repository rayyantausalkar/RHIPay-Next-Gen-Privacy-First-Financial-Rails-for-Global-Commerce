...
export const UpiPinModal: React.FC<UpiPinModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-sm bg-[#08131d] border border-emerald-500/20 rounded-3xl p-6 shadow-2xl shadow-emerald-950/40 text-white flex flex-col items-center">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        {/* Bank & Account Pill */}
        {user && (
          <div className="mt-3 px-3 py-1 bg-white/[0.03] border border-white/[0.08] rounded-full text-[11px] text-zinc-300 flex items-center gap-1.5 font-mono">
        ...
        <div className="grid grid-cols-3 gap-2.5 w-full max-w-[280px]">
          {["1", "2", "3", "4", "5", "6", "7", "8", "9"].map((num) => (
            <button
              ...
              className="h-13 sm:h-14 rounded-2xl bg-white/[0.04] hover:bg-emerald-500/15 border border-white/[0.06] hover:border-emerald-500/30 text-xl font-bold text-zinc-100 hover:text-emerald-300 transition-all active:scale-95 flex items-center justify-center shadow-sm"
            >
              {num}
            </button>
          ))}
          ...
          <button
            ...
            className="h-13 sm:h-14 rounded-2xl bg-white/[0.02] hover:bg-white/[0.06] border border-white/[0.04] text-xs font-semibold text-zinc-400 hover:text-zinc-200 transition-all active:scale-95 flex items-center justify-center"
          >
            Clear
          </button>
          ...
          <button
            ...
            disabled={isLoading || pin.length === 0}
            className="h-13 sm:h-14 rounded-2xl bg-white/[0.02] hover:bg-rose-500/10 border border-white/[0.04] hover:border-rose-500/20 text-zinc-400 hover:text-rose-400 transition-all active:scale-95 flex items-center justify-center"
          >
        ...
        <div className="mt-5 flex items-center gap-1.5 text-[11px] text-zinc-500">
          ...
          <span>Zero-Knowledge Authenticated Banking Gate</span>
        </div>
      </div>
    </div>
  );
