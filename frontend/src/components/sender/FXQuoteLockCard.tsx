...
export const FXQuoteLockCard: React.FC<FXQuoteLockCardProps> = ({
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-[#09090b] border border-white/[0.08] rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-2xl relative overflow-hidden">
      ...
      <div className="mt-4 p-3.5 rounded-2xl bg-zinc-950 border border-white/[0.08] relative overflow-hidden">
        <div className="flex items-center justify-between text-xs mb-2">
          <div className="flex items-center gap-1.5 text-zinc-300 font-medium">
            <Clock className={`w-3.5 h-3.5 ${isExpired ? "text-rose-400" : "text-emerald-400"}`} />
            <span>
              {isExpired ? "Quote Expired" : `Quote Guaranteed for ${formatTime(secondsRemaining)}`}
            </span>
          </div>
      ...
      <div className="mt-4 p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/25 flex items-center gap-2.5">
      ...
      <div className="mt-4 p-5 rounded-3xl bg-zinc-950 border border-white/[0.08] space-y-4">
        {/* Origin Payer Debit */}
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl sm:text-3xl font-extrabold text-white font-mono tracking-tight">
              ...
              <span className="text-sm font-bold text-emerald-400">{quote.origin_currency}</span>
            </div>
          </div>
        ...
        <div className="relative flex items-center justify-center my-1">
          ...
          <div className="absolute p-1.5 rounded-full bg-zinc-900 border border-white/10 text-emerald-400">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl sm:text-3xl font-extrabold text-white font-mono tracking-tight">
              ...
              <span className="text-sm font-bold text-emerald-400">{quote.destination_currency}</span>
            </div>
          </div>
          <div className="text-right text-xs font-mono text-zinc-400">
            Spoke B
      ...
      <div className="mt-4 p-4 rounded-2xl bg-zinc-950/60 border border-white/[0.06] space-y-2 text-xs">
        ...
        <div className="pt-2 border-t border-white/[0.06] flex items-center justify-between text-[10px] font-mono text-zinc-500">
          <span>Quote ID:</span>
          <span className="truncate max-w-[200px]">{quote.quote_id}</span>
        </div>
      </div>
      ...
      <div className="mt-5 space-y-2">
        {isExpired ? (
          <button
            ...
            onClick={handleRelockRate}
            disabled={isRefreshing}
            className="w-full py-4 px-6 rounded-2xl bg-amber-500 hover:bg-amber-400 text-black font-bold text-sm shadow-xl shadow-amber-500/25 flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
          >
          ...
          >
            ...
            <span>Lock Rate & Proceed to ZK Proof (&lt;1.2s)</span>
