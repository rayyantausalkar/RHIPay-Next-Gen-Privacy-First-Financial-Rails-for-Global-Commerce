    ...
    const runAssembly = async () => {
      ...
      try {
        const res = await assemblePacs008({
          ...
          zk_proof: {
            ...(zkProof.proof || {}),
            merkle_root: zkProof.merkle_root,
            nullifier_hash: nullifier.nullifier_hash || zkProof.nullifier_hash,
        ...
        <div className="space-y-3.5 sm:space-y-4 my-2 sm:my-3 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-2 gap-2 sm:gap-2.5 text-xs">
            <div className="p-2.5 sm:p-3 rounded-2xl bg-zinc-950 border border-white/[0.06] min-w-0">
              <span className="text-[10px] font-medium text-zinc-500 uppercase block mb-0.5 truncate">
                Instructed Debit
              </span>
              <span className="font-extrabold text-xs sm:text-sm text-white font-mono truncate block">
            ...
            <div className="p-2.5 sm:p-3 rounded-2xl bg-zinc-950 border border-white/[0.06] min-w-0">
              ...
              <span className="font-extrabold text-xs sm:text-sm text-emerald-400 font-mono truncate block">
          ...
          <div className="p-3 rounded-2xl bg-zinc-950 border border-white/[0.06] space-y-1.5 text-xs font-mono">
            <div className="flex items-center justify-between text-[11px] gap-2">
              <span className="text-zinc-400 font-sans flex-shrink-0">UETR:</span>
              <span className="text-emerald-400 font-bold truncate text-right">
                {pacsMessage.uetr}
              </span>
            </div>
            <div className="flex items-center justify-between text-[11px] gap-2">
              <span className="text-zinc-400 font-sans flex-shrink-0">Message Id:</span>
          ...
          <div className="rounded-2xl bg-zinc-950 border border-white/[0.08] overflow-hidden">
            <div className="flex items-center justify-between p-2 px-3 border-b border-white/[0.06] bg-black/40">
              ...
              <button
                ...
                className="flex items-center gap-1 text-[11px] font-mono text-zinc-400 hover:text-emerald-400 transition-colors active:scale-95"
              >
            ...
            <pre className="p-3 text-[10px] font-mono text-emerald-300/90 max-h-40 sm:max-h-44 overflow-y-auto overflow-x-auto leading-relaxed select-all">
              {activeTab === "xml"
                ? pacsMessage.xml_payload
                : JSON.stringify(pacsMessage.canonical_json, null, 2)}
            </pre>
          ...
          >
            ...
            <span className="truncate">Submit to Nexus Hub for Final Settlement</span>
